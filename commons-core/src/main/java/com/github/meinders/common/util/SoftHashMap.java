...
/**
 * Equivalent to {@link java.util.WeakHashMap}, except that it uses a
 * {@link SoftReference} instead of a {@link WeakReference} to reference
 * entries.
 */
public class SoftHashMap<K, V> extends AbstractMap<K, V> implements Map<K, V> {
	 ...
	 * @param loadFactor The load factor of the <tt>SoftHashMap</tt>
	 ...
	 * specified map. The <tt>SoftHashMap</tt> is created with the default
	 ...
	 * <tt>null</tt> if the map contained no mapping for the key. A return
	 * value of <tt>null</tt> does not <i>necessarily</i> indicate that the
	 * map contained no mapping for the key; it's also possible that the map
	...
	private static class Entry<K, V> extends SoftReference<K> implements
		...
		public int hashCode() {
			Object k = getKey();
			Object v = getValue();
			return ((k == null ? 0 : k.hashCode()) ^ (v == null ? 0
			        : v.hashCode()));
		}
