...
class GNUDiff implements Diff
{
	...
	private void setInput(Collection<? extends Object> a,
	        Collection<? extends Object> b)
	{
		Hashtable h = new Hashtable(a.size() + b.size());
	...
	public List<Change> diff(boolean reverse)
	{
		List<Change> result = new ArrayList<Change>();

		change currentChange = diff_2(reverse);
		while (currentChange != null)
		{
			result.add(new ChangeImpl(currentChange.line0,
			        currentChange.deleted, currentChange.line1,
	...
	class file_data
	{
		...
		file_data(Collection<?> data, Hashtable h)
		{
			buffered_lines = data.size();
			...
			{
				Integer ir = (Integer) h.get(dataObject);
				if (ir == null)
					h.put(dataObject, new Integer(equivs[i] = equiv_max++));
				else
					equivs[i] = ir.intValue();
				i++;
			}
